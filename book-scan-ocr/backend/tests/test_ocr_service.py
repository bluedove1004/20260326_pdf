"""Unit tests for OCRService (EasyOCR) result parsing."""
...
def _make_ocr_line(text: str, confidence: float, pts=None):
    ...
    # EasyOCR format: (pts, text, confidence)
...
def test_parse_empty_result():
    """Empty EasyOCR output should produce an 'empty' PageResult."""
    service = OCRService.__new__(OCRService)
    # We don't need to mock readers for _parse_result, it just takes raw list
